...
class Gfheddhjek:
    ...
    async def search_relevant_items(item: str, session: aiohttp.ClientSession) -> Union[list, None]:
        ...
        try:
            logging.warning(f"Sent to " + Gfheddhjek.BASE_URL)
            req = await session.get(Gfheddhjek.SEARCH_URL.format(item), ssl=False)
            res = await req.text()
            logging.warning(f"Got from " + Gfheddhjek.BASE_URL)
            doc = BeautifulSoup(res, "html.parser")

            # check if the page is loaded correctly. If not, try getting it through the browser
            check = get_check(doc)
            if not check:
                logging.warning(f"BROWSER IS WORKING IN " + Gfheddhjek.BASE_URL)
                res = Browser(Gfheddhjek.SEARCH_URL.format(item)).get_page(cls_wait_tag=check_cls_tag)
                doc = BeautifulSoup(res, "html.parser")

            # get the list of items from the page
            items_found = get_catalog(doc)
            if not items_found:
                logging.warning(f"There are no items in " + Gfheddhjek.BASE_URL)
                return None
            ...
            logging.warning("ERROR: {error} IN: " + Gfheddhjek.BASE_URL)
            return None
    ...
    async def get_item_info(link: str, session: aiohttp.ClientSession) -> Union[dict, None]:
        ...
        # The algorithm of the search is implemented here. USUALLY IT DOESN'T CHANGE
        if Gfheddhjek.BASE_URL not in link:
            logging.warning(f"WRONG LINK PROVIDED FOR " + Gfheddhjek.BASE_URL)
            return None
        try:
            logging.warning(f"Sent to " + Gfheddhjek.BASE_URL)
...
async def test_search(item, link=None):
    async with aiohttp.ClientSession() as session:
        result = await Gfheddhjek.search_relevant_items(item, session)  # link[1] is the url of the item
        result_link = await Gfheddhjek.get_item_info(link, session) if link else None
