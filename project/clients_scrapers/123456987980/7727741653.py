...
class Kkfkkhejig:
    ...
    async def search_relevant_items(item: str, session: aiohttp.ClientSession) -> Union[list, None]:
        try:
            logging.warning(f"Sent to " + Kkfkkhejig.BASE_URL)
            req = await session.get(Kkfkkhejig.SEARCH_URL.format(item))
            res = await req.text()
            doc = BeautifulSoup(res, "html.parser")
            logging.warning(f"Got from " + Kkfkkhejig.BASE_URL)
            ...
            if not check:
                logging.warning(f"BROWSER IS WORKING IN " + Kkfkkhejig.BASE_URL)
                res = Browser(Kkfkkhejig.SEARCH_URL.format(item)).get_page("CHOOSE_YOUR_CHECK_CLASS")
                doc = BeautifulSoup(res, "html.parser")

            items_found = OperationalTools.operate(lambda: None)
            if not items_found:
                logging.warning(f"There are no items in " + Kkfkkhejig.BASE_URL)
                return None
            ...
            logging.warning("ERROR: {error} IN: " + Kkfkkhejig.BASE_URL)
            return None

    @staticmethod
    async def get_item_info(link: str, session: aiohttp.ClientSession) -> Union[dict, None]:
        if  Kkfkkhejig.BASE_URL not in link:
            logging.warning(f"WRONG LINK PROVIDED FOR " + Kkfkkhejig.BASE_URL)
            return None
        try:
            logging.warning(f"Sent to " + Kkfkkhejig.BASE_URL)
...
async def test_search(item):
    async with aiohttp.ClientSession() as session:
        result = await Kkfkkhejig.search_relevant_items(item, session)
        return result
