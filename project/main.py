...
from project.async_search import (
    run_search_link,
    run_search_all_items,
    run_search_all_links,
    run_search_item,
)
...
def profile():
    ...
    if request.method == "POST":
        ...
        if not item_link:
            item_link = db_get_item_link_new(
                user_id=user_id, company_inn=_inn, item_name=item_name
            )
    ...
    competitors = [
        ...
        if competitor.competitor_inn != current_user.company_inn
    ]
    all_items = db_get_items(user_id)
    own_items = list(
        filter(lambda x: inn_checker(x["competitor_inn"]) == _inn, all_items)
    )
    ...
    for item in own_items:
        ...
        for linked_item in all_linked_items:
            if item["link"] == linked_item.item_link:
                ...
                related[comp_inn] = {
                    "url": linked_item.connected_item_link,
                    "name": con_item["name"],
                }
...
def company_goods():
    ...
    if request.method == "POST":
        ...
        if not item_link:
            return render_template(
                "company-goods.html", competitors=available_competitors, items=items
            )
        competitor_inn = None
        ...
        if not competitor_inn or competitor_inn not in [
            competitor.competitor_inn for competitor in available_competitors
        ]:
            return render_template(
                "company-goods.html", competitors=available_competitors, items=items
            )
...
def comparison():
    ...
    all_items = db_get_items(user_id)
    own_items = list(
        filter(lambda x: inn_checker(x["competitor_inn"]) == user_inn, all_items)
    )
    all_linked_items = db_get_users_connections(user_id)
    ...
    for item in own_items:
        ...
        for linked_item in all_linked_items:
            if linked_item.item_link == item_url:
                ...
                cr_item = db_get_item(
                    user_id=user_id, item_link=linked_item.connected_item_link
                )
                info["cr_prices"][cr_inn] = cr_item
        cr_prices = [
            item["last_price"]
            for item in info["cr_prices"].values()
    ...
    return render_template(
        "comparison.html", items=own_items, items_info=items_info, competitors=crs
    )
...
def price_looker():
    ...
    if request.method == "POST":
        ...
        if not item:
            return render_template(
                "price-looker-results.html", competitors=available_competitors
            )
        ...
        if not chosen_competitors:
            return render_template(
                "price-looker.html", competitors=available_competitors
            )

        min_price = check_price(request.form.get("min_price"))
        max_price = check_price(request.form.get("max_price"))
        result = run_search_all_items(
            current_user.get_id(), item=item, chosen_comps=chosen_competitors
        )
        result = format_search_all_result(
            item, result, available_competitors, min_price, max_price
        )
        for r in result:
    ...
    if request.method == "GET":
        ...
        if item_search_field:
            ...
            result = run_search_all_items(
                current_user.get_id(), item=item_search_field, chosen_comps=chosen_comps
            )
            result = format_search_all_result(
                item_search_field, result, available_competitors
            )
            return render_template(
                "price-looker_layout.html",
...
def request_connection(com_inn):
    ...
    if db_update_con_status(user_id, com_inn):
        ...
        db_add_scraper(
            user_inn=current_user.company_inn, comp_inn=current_user.company_inn
        )
        print("scr made")
        if "profile" in request.referrer:
            return redirect(url_for("main.profile"))
...
def change_web():
    ...
    _inn = current_user.company_inn
    available_inns = [
        competitor.competitor_inn for competitor in db_get_competitors(user_id)
    ]
    ...
    if request.method == "GET":
        ...
        if (
            not requested_connection
            or requested_connection.connection_status == "disconnected"
        ):
...
def link_items():
    ...
    available_inns = [
        competitor.competitor_inn
        for competitor in db_get_competitors(user_id, connection_status="connected")
    ]
    if comp_inn not in available_inns:
        return "This comp isn't connected"
    new_link = get_link(request.form.get("new_link"))
    old_link = db_get_item_connection(
    ...
    if not new_link:
        if old_link:
            db_delete_connection(
                user_id=user_id,
                item_link=item_link,
                linked_item_link=old_link.connected_item_link,
            )
        return "deleted"
    if not db_add_item(user_id=user_id, company_inn=comp_inn, link=new_link):
        return "Cannot get item info!"
    if db_add_item_connection(
        user_id=user_id,
        item_link=item_link,
        connected_item_link=new_link,
        comp_inn=comp_inn,
    ):
...
def autoload_associations():
    ...
    for item in own_items:
        ...
        for inn in all_inns:
            if db_get_item_connection(
                user_id=user_id, user_link=item["link"], comp_inn=inn
            ):
        ...
        for result in search:
            ...
            last_relevance = (
                item_con[competitor_inn]["relevance"]
                if item_con.get(competitor_inn)
                else 0
            )
            if (
                result["relevance"] > MIN_RELEVANCE
                and result["relevance"] >= last_relevance
            ):
