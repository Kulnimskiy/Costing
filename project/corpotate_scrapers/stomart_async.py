    ...
    async def search_relevant_items(item: str, session: aiohttp.ClientSession):
        ...
        try:
            req = await session.get(
                Stomshop.SEARCH_URL.format(item), ssl=False
...
class Dentikom:
    ...
    async def search_relevant_items(item: str, session: aiohttp.ClientSession):
        try:
            req = await session.get(
                Dentikom.SEARCH_URL.format(item), headers=Dentikom.headers
            )
            res = await req.text()
            doc = BeautifulSoup(res, "html.parser")
            items_found_raw = operate(
...
class Dentex:
    ...
    async def search_relevant_items(item: str, session: aiohttp.ClientSession):
        try:
            ...
            items_found_raw = doc.find("div", class_="short-search").find_all(
